var dateFormat = require('./date_format')
  ...
  , layoutMakers = {
    ...
    , "colored": function(config) {
    var pattern = config.pattern || undefined;
    return colouredLayout(pattern);
      }
    , "coloured": function(config) {
    var pattern = config.pattern || undefined;
    return colouredLayout(pattern);
      }
...
function formatLogData(logData) {
    ...
    // Use this to place 2nd argument onwards on a new line
    ...
    data.forEach(function (item) {
        ...
        first = false;
        // TODO: Make inspect settings configurable
...
function timestampLevelAndCategory(loggingEvent, colour, pattern) {
    var pattern = pattern || '[%s] [%s] %s - '
    //var output = colorize(
    //    formatLogData(
    //        pattern
    ...
    //);
    var output = colorize(
        patternLayout(pattern, true)(loggingEvent)
    , colour)
    return output;
}
 ...
 */
function colouredLayout (pattern) {
    return function(loggingEvent) {
      return timestampLevelAndCategory(loggingEvent, colours[loggingEvent.level.toString()], pattern) + formatLogData(loggingEvent.data);
    }
}
 ...
 * @author Stephan Strittmatter
 */
function patternLayout (pattern, useColors) {
    ...
    if (conversionCharacter === 'p') {
      replacement = colorize(replacement, colours[loggingEvent.level.toString()]);
    }
